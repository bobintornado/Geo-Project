...
function yaiqngDistance (lon1, lat1, lon2, lat2) {
...
KDE = function(theUrl, usSpinnerService) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            // format
            for (var i = Global_KDE_data['x'].length - 1; i >= 1000; i--) {
                var latlng_point = {
...
calculate_buffer = function(source, target, buffer_radius) {
    ...
    for (var i = 0; i < target_array.length; i++) {
        for (var j = 0; j < source_array.length; j++) {
			...
			var d = yaiqngDistance(target_array[i].lng,target_array[i].lat,source_array[j].lng,source_array[j].lat);
            if (d < buffer_radius) {
