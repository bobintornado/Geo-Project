...
function uploadGeoJson(id){
	...
	$.ajax({
		url : '/GEO/UploadServlet?json={"fileType":"js,json,geojson,text","uploadDirectory":"geo","folderName":"geojson"}',
		type : "POST",
		data : fd,
		dataType : 'json',
		processData : false,
...
function submitGEOJson(){
	...
	$.ajax({
		url : '/GEO/UploadGEOJSONFile?json='+inputJson,
		type : 'GET',
		dataType : 'json',
		error : function(err){
			console.log(err);
...
function deleteJsonFile(id){
	...
	if(go){
		...
		$.ajax({
			url : '/GEO/DeleteGeoJSONFile?json='+inputJson,
			type : 'GET',
			dataType : 'json',
			error : function(err){
				console.log(err);
